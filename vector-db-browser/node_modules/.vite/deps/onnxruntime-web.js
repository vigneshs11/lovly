...
`))[0] == "Error" && y.shift(), xi(y);
    ...
    (async function() {
      ...
      lt ??= n.locateFile ? n.locateFile ? n.locateFile("ort-wasm-simd-threaded.jsep.wasm", _) : _ + "ort-wasm-simd-threaded.jsep.wasm" : new URL("ort-wasm-simd-threaded.jsep.wasm", import.meta.url).href;
      try {
  ...
  }), Kp = Sa, jp = globalThis.self?.name?.startsWith("em-pthread");
  jp && Sa();
});
var ka;
var Hn;
...
var yt = U(() => {
  ...
  qn = false, kr = false, Da = false, Jp = () => {
    ...
    let o = e.wasmPaths, i = typeof o == "string" ? o : void 0, a = o?.mjs, u = a?.href ?? a, d = o?.wasm, c = d?.href ?? d, p = e.wasmBinary, [m, f] = await za(u, i, r > 1), b = false, g = [];
...
var Ma = U(() => {
  ...
  Ba = (e) => {
    ...
    try {
      ...
      let i = 0;
      return e?.tag !== void 0 && (i = Ne(e.tag, n)), r = t._OrtCreateRunOptions(o.logSeverityLevel, o.logVerbosityLevel, !!o.terminate, i), r === 0 && fe("Can't create run options."), e?.extra !== void 0 && Zt(e.extra, "", /* @__PURE__ */ new WeakSet(), (a, u) => {
  ...
  Ha = /* @__PURE__ */ new Map([["float32", 32], ["float16", 16], ["int32", 32], ["uint32", 32], ["int64", 64], ["uint64", 64], ["int8", 8], ["uint8", 8], ["int4", 4], ["uint4", 4]]), Xn = (e, t) => {
    ...
    async download(t) {
      if (this.activeUpload) {
        let r = this.wrapper?.isDataConverted ? Fa(this.activeUpload, this.wrapper?.type) : this.activeUpload;
        if (t) {
...
var Za = U(() => {
  ...
  Nr = /* @__PURE__ */ new Map([[1, "float32"], [10, "float16"], [6, "int32"], [12, "uint32"], [7, "int64"], [13, "uint64"], [22, "int4"], [21, "uint4"], [3, "int8"], [2, "uint8"], [9, "uint8"]]), cm = (e, t) => {
    ...
    isGraphInputOutputTypeSupported(t, r, n = true) {
      ...
      return typeof i > "u" ? false : n ? !!o?.opSupportLimits().input.dataTypes.includes(i) : !!o?.opSupportLimits().output.dataTypes.includes(i);
    }
    flush() {
    }
  };
    ...
    let i = e.map((p) => p.length).map((p, m) => P(`input${m}`, t, p)), a = E.size(n), u = M("output", t, n.length), d = [...r.symbolToInfo.keys()].filter((p) => !r.rhs.symbolToIndices.has(p)), c = (p) => {
      ...
      r.symbolToInfo.forEach((k, I) => {
        if (r.rhs.symbolToIndices.has(I)) {
          let O = r.rhs.symbolToIndices.get(I)?.[0];
...
var tc = U(() => {
  ...
  mg = (e, t) => {
    ...
    let n = e.name;
    return e.shaderCache?.hint && (n += "[" + e.shaderCache.hint + "]"), n += ":" + r + `:${mg(t, e.shaderCache?.inputDependencies ?? new Array(t.length).fill("dims"))}`, n;
  }, Io = class {
    constructor(t) {
    ...
    flush() {
      ...
      this.queryType !== "none" && (this.commandEncoder.resolveQuerySet(this.querySet, 0, this.pendingDispatchNumber * 2, this.queryResolveBuffer, 0), t = this.device.createBuffer({ size: this.pendingDispatchNumber * 2 * 8, usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST }), this.pendingQueries.set(t, this.pendingKernels), this.pendingKernels = [], this.commandEncoder.copyBufferToBuffer(this.queryResolveBuffer, 0, t, 0, this.pendingDispatchNumber * 2 * 8)), this.device.queue.submit([this.commandEncoder.finish()]), this.gpuDataManager.refreshPendingBuffers(), this.commandEncoder = null, this.pendingDispatchNumber = 0, this.queryType !== "none" && t.mapAsync(GPUMapMode.READ).then(() => {
        ...
        for (let o = 0; o < r.length / 2; o++) {
          ...
          if (this.env.webgpu.profiling?.ondata) this.env.webgpu.profiling.ondata({ version: 1, inputsMetadata: m.map(($) => ({ dims: $.dims, dataType: Xe($.dataType) })), outputsMetadata: f.map(($) => ({ dims: $.dims, dataType: Xe($.dataType) })), kernelId: a, kernelType: d, kernelName: c, programName: p, startTime: _, endTime: S });
          else {
            let $ = "";
            m.forEach((x, T) => {
...
var nc = U(() => {
  ...
  rr = class e {
    ...
    compute(t, r) {
      let n = r?.inputs?.map((u) => typeof u == "number" ? this.inputs[u] : u) ?? this.inputs, o = r?.outputs ?? [], i = (u, d, c) => new rr(this.module, d, this.output(u, c), c), a = (u, d) => {
        let c = wt(u, d);
  ...
  gg = (e, t) => {
    ...
    try {
      ...
      for (let T of t?.executionProviders ?? []) if ((typeof T == "string" ? T : T.name) === "webnn") {
        if (o.shouldTransferToMLTensor = false, typeof T != "string") {
          let I = T, O = I?.context, z = I?.gpuDevice, B = I?.deviceType, W = I?.powerPreference;
      ...
      i = await o._OrtCreateSession(r, n, a), o.webgpuOnCreateSession?.(i), i === 0 && fe("Can't create a session."), o.jsepOnCreateSession?.(), o.currentContext && (o.webnnRegisterMLContext(i, o.currentContext), o.currentContext = void 0, o.shouldTransferToMLTensor = true);
      let [m, f] = bg(i), b = !!t?.enableGraphCapture, g = [], _ = [], S = [], $ = [], w = [];
      for (let T = 0; T < m; T++) {
        let [k, I, O] = oc(i, T);
      ...
      for (let T = 0; T < f; T++) {
        ...
        _.push(z), $.push(I === 0 ? { name: z, isTensor: false } : { name: z, isTensor: true, type: Xe(I), shape: O });
        {
          ...
          let B = typeof t?.preferredOutputLocation == "string" ? t.preferredOutputLocation : t?.preferredOutputLocation?.[z] ?? "cpu", W = o.webnnIsGraphOutput;
          if (B === "cpu" && W && W(i, z)) {
      ...
      o._free(r), a !== 0 && o._OrtReleaseSessionOptions(a) !== 0 && fe("Can't release session options."), d.forEach((m) => o._free(m)), o.unmountExternalData?.();
    }
  }, Tr = (e) => {
    let t = ge(), r = Pt.get(e);
    ...
    try {
      ...
      if (f && !g) {
        ...
        for (let H = 0; H < S; H++) {
          let ae = n[H];
          o[H]?.[3] ? a._OrtBindOutput(V, m[ae], T[H], 0) !== 0 && fe(`Can't bind pre-allocated output[${H}] for session=${e}.`) : a._OrtBindOutput(V, m[ae], 0, J[ae]) !== 0 && fe(`Can't bind output[${H}] to ${de[H]} for session=${e}.`);
        }
      ...
      for (let V = 0; V < S; V++) {
        ...
        try {
          ...
          Z = Xe(ee);
          let ze = f?.outputPreferredLocations[n[V]];
          if (Z === "string") {
      ...
      a.webnnOnRunEnd?.(c), a.stackRestore(I), x.forEach((q) => a._OrtReleaseTensor(q)), T.forEach((q) => a._OrtReleaseTensor(q)), k.forEach((q) => a._free(q)), $ !== 0 && a._OrtReleaseRunOptions($), w.forEach((q) => a._free(q));
    }
  }, Cr = (e) => {
    let t = ge(), r = Pt.get(e);
